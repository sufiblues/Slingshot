...
/*** Calculates the manhattan distance of a line. EX. (1,1),(3,4) has a manhattan distance of (2,3) ***/ 
glm::vec2 manhattanDistance(Line line){
    return glm::vec2(line.two[0] - line.one[0], line.two[1] - line.one[1]);
}
/*** Calculates a vector that is perpendicular to the line, NOTE! POSITION OF VECTOR IS NOT SPECIFIED ***/
glm::vec2 perpendicularVector(Line line){	
    return glm::vec2(line.two[1] - line.one[1], -(line.two[0] - line.one[0]));
}

/*** Pads line with a scalar amount from center out, NOTE! RETURNED LINE IS A NON TRUNCATED FLOAT ***/
...
/*** Prints Coordinates of a line to the console with an identifier. TODO: Could add more information about line like slope etc ***/
...
/*** Creates a set of lines that bounds the center of the rectangle to the Line. TODO: NEED TO CREATE A BETTER PAD LENGTH ***/
...
/*** Cast a ray from a point to a line and where the time of impact occurs ***/
...
/*** See if a rectangle collides with a set of boundires ***/
...
/*** See if a rectangle collides with a boundry ***/
glm::vec2 Cast(Rectangle rect, glm::vec2 ray, Line boundry){
	...
	for (int i = 0; i < outline.size(); i++){
	    glm::vec2 temp = rayCast(rect.center, ray, outline[i]);
	    if (temp[0] < 1 && temp[0] >= 0){ 
	       if (temp[0] < tandu[0]){
	           tandu = temp;
	       }
...
void RenderShape(std::vector<Line> lines, SDL_Color color, int camera_x, int camera_y){
    for (int i = 0; i < lines.size(); i++){
        RenderShape(lines[i],color,camera_x, camera_y);
    }
}
...
void RenderShape(Line line, SDL_Color color, int camera_x, int camera_y){
    ...
    SDL_RenderDrawLine(Renderer, line.one[0] - camera_x ,line.one[1] - camera_y ,line.two[0] - camera_x,line.two[1] - camera_y);
}
